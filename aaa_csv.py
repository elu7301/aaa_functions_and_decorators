...
def file_open(path: str) -> list:
    """
    Opens a CSV file at the given path and returns its contents as a list of lists.

    :param path: A string representing the file path of the CSV file to be opened.
...
def get_data(path: str) -> dict:
    """
    Parses a CSV file containing information about employees and returns a dictionary
    containing information about departments.

    :param path: A string representing the file path of the CSV file to be parsed.
    :return: A dictionary containing information about departments.
    """
    data = file_open(path)[1:]
    departments = {employer[1]: [set(), 1e6, 0, 0, 0] for employer in data}
    for row in data:
        departments[row[1]][0].add(row[2])
        departments[row[1]][1] = float(row[5]) if float(row[5]) < departments[row[1]][1] else departments[row[1]][1]
        departments[row[1]][2] = float(row[5]) if float(row[5]) > departments[row[1]][2] else departments[row[1]][2]
...
def find_departments(data: dict) -> None:
    ...
    :param data: A dictionary containing information about departments and their employees.
    :return: None
    """
...
def department_report(data: dict) -> None:
    """
    Prints a summary report of departments, including the number of employees, minimum and maximum salaries, and average salary.
...
def save_report(data: dict) -> None:
    """
    Saves a summary report of departments to a CSV file, including the number of employees, minimum and maximum salaries,
    and average salary.
    ...
    with open('department_report.csv', 'w', newline='') as csvfile:
        ...
        writer.writerow(['Department', 'Number of Employees', 'Minimum Salary', 'Maximum Salary', 'Average Salary'])
        for key, value in data.items():
            writer.writerow([key, value[3], value[1], value[2], round(value[4] / value[3], 2)])
